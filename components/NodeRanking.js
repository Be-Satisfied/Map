...
const NodeRanking = ({ nodes }) => {
  ...
  return (
    <div
      className="w-full md:w-2/5 h-[600px] p-6 rounded-[20px] flex flex-col box-shadow box-border"
      style={{
        background: "rgba(255, 255, 255, 0.50)",
        backdropFilter: "blur(10px)",
      }}
    >
      <h2 className="text-2xl font-semibold mb-6 border-b text-black border-[#BABABA] pb-3 flex flex-row items-center gap-3">
        <svg
      ...
      >
        <div className="flex flex-row items-center w-full shadow-sm">
          <input
            ...
            className="w-full transition-all duration-200 focus:ring-2 focus:ring-[#495EDB] focus:ring-opacity-50"
            style={{
              width: "100%",
              borderWidth: "1.5px",
              borderColor: "#495EDB",
          ...
          <button
            ...
            className="flex items-center justify-center text-white cursor-pointer hover:bg-[#3a4ccc]"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="28"
      ...
      <div className="mt-2 space-y-3 flex-1 node-ranking-list pr-2">
        {filteredNodes &&
          filteredNodes.map((node, index) => (
            <div
              key={node.code}
              className={`flex flex-col w-full p-3 rounded-xl transition-all duration-300 hover:bg-white hover:shadow-lg  ${
                index % 2 === 0 ? "bg-[#e3e9ff]" : "bg-[#f5f7ff]"
              }`}
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <span className="text-white font-medium bg-[#495EDB] rounded-full w-[32px] h-[32px] overflow-clip flex items-center justify-center shadow-sm transition-transform hover:scale-110">
                    {index + 1}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[#495EDB] text-xl font-semibold">
                    {node.count}
                  </span>
