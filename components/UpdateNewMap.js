...
function WorldMap({ nodes = {} }) {
  ...
  // Initialize region nodes and calculate counts in useEffect
  useEffect(() => {
    // Initialize region nodes
    const initialRegionNodes = Object.entries(REGIONS).reduce(
    ...
    const updatedRegionNodes = { ...initialRegionNodes };
    Object.entries(nodes).forEach(([countryCode, count]) => {
      for (const [region, countries] of Object.entries(REGIONS)) {
        if (countries.includes(countryCode)) {
          updatedRegionNodes[region] += count;
          break;
        }
      }
    });

    setRegionNodes(updatedRegionNodes);
  }, [nodes]);
    ...
    >
      ...
      >
        {Object.entries(regionNodes).map(([region, count]) => (
          <div
            key={region}
            className="absolute flex flex-row md:flex-col items-center p-1 bg-[rgba(255,255,255,0.8)] text-black rounded-lg text-[8px] md:text-base cursor-pointer hover:bg-[rgba(255,255,255,0.9)]"
            style={{
              ...getRegionPosition(region),
              zIndex: 1,
            }}
            onClick={(e) => {
              // Reset all other elements to z-index 1
              document.querySelectorAll("#chartdiv > div").forEach((el) => {
                el.style.zIndex = "1";
              });
              // Set clicked element to z-index 10
...
const getRegionPosition = (region) => {
  const positions = {
    ...
    AFS: { left: "50%", top: "85%" }, // South Africa
